...
describe('Invite user', () => {
  ...
  test('Logs invited users and the teams they belong to', async () => {
    ...
    expect(consoleLog).toBeCalledWith(
      `Invited user ${fetchUsersWithTeamsResponse.items[0].data.email.iv} (${listTeamResponse.items[0].data.displayName.iv})`,
    );
    expect(consoleLog).toBeCalledWith(
      `Invited user ${fetchUsersWithTeamsResponse.items[1].data.email.iv} (${listTeamResponse.items[1].data.displayName.iv})`,
    );
  });
