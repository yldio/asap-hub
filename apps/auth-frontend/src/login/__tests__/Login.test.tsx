...
it('generates production links when redirect uri not present', () => {
  mockLocation.mockReturnValue(
    new URL('?response_type=code&screen_hint=signup', mockLocation()),
  );
  const { getByText } = render(<Login email="" setEmail={() => {}} />);
  expect(getByText('Terms', { selector: 'a' })).toHaveAttribute(
    'href',
    'https://hub.asap.science/terms-and-conditions',
  );
});

it('generates links for environment when redirect present', () => {
  mockLocation.mockReturnValue(
    new URL(
  ...
  expect(getByText('Terms', { selector: 'a' })).toHaveAttribute(
    'href',
    'https://dev.hub.asap.science/terms-and-conditions',
  );
});

it('generates links for environment when deep redirect present', () => {
  mockLocation.mockReturnValue(
    new URL(
      '?response_type=code&screen_hint=signup&redirect_uri=https%3A%2F%2Fdev.hub.asap.science%2Fprotected',
      mockLocation(),
    ),
  );
  const { getByText } = render(<Login email="" setEmail={() => {}} />);
...
it('generates links preserving port when specified in redirect', () => {
  mockLocation.mockReturnValue(
    new URL(
      '?response_type=code&screen_hint=signup&redirect_uri=http%3A%2F%2Flocalhost%3A3000',
      mockLocation(),
    ),
  );
  const { getByText } = render(<Login email="" setEmail={() => {}} />);
