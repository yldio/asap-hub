...
import { useAnalyticsPreprintCompliance } from '../state/preprint-compliance.state';
...
describe('useAnalyticsPreprintCompliance', () => {
  it('maps a non-Error rejection to an empty list', async () => {
    (getPreprintCompliance as jest.Mock).mockRejectedValue('nope');

    const { result } = renderStateHook(() =>
      useAnalyticsPreprintCompliance(options),
    );
  ...
  it('maps an undefined result to null via the ?? null branch', async () => {
    (getPreprintCompliance as jest.Mock).mockResolvedValue(undefined);

    const { result } = renderStateHook(() =>
      useAnalyticsPreprintCompliance(options),
    );

    await waitFor(() => expect(getPreprintCompliance).toHaveBeenCalled());
    expect(result.current).toBeNull();
  });

  it('defaults a missing timeRange to "all"', async () => {
    (getPreprintCompliance as jest.Mock).mockResolvedValue({
      total: 0,
      items: [],
    });
  ...
  it('re-throws Error rejections to the error boundary', async () => {
    const consoleErrorSpy = jest
      .spyOn(console, 'error')
      .mockImplementation(() => {});
    (getPreprintCompliance as jest.Mock).mockRejectedValue(new Error('boom'));
