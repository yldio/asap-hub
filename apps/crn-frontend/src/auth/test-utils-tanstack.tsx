...
const createAuth0ForQuery = (
  ...
  const baseAuth0: Auth0 = {
    loading: !auth0Client && !hasAuth0User,
    popupOpen: false,
  ...
  const authenticatedOverrides: Partial<Auth0> = auth0User
    ? {
...
export const Auth0ProviderForQuery: React.FC<{
  ...
  useEffect(() => {
    ...
    const initAuth0 = async () => {
      ...
      if (!cancelled) {
        setAuth0(createAuth0ForQuery(auth0Client, user, auth0Overrides));
      }
    };
  ...
  return (
    <Auth0ContextCRN.Provider
      value={
        auth0?.loading === false
          ? auth0
          : createAuth0ForQuery(undefined, user, auth0Overrides)
      }
    >
      {children}
    </Auth0ContextCRN.Provider>
...
export const WhenReadyForQuery: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  const contextAuth0 = useContext(Auth0ContextCRN);
  return contextAuth0.loading ? <p>Auth0 loading...</p> : <>{children}</>;
};
