export const GTM_CONTAINER_ID = process.env.REACT_APP_GTM_CONTAINER_ID;
export const API_BASE_URL =
  process.env.REACT_APP_API_BASE_URL || 'http://localhost:3333';
export const ALGOLIA_APP_ID =
  process.env.REACT_APP_ALGOLIA_APP_ID || 'LVYWOPQ0A9';
export const ALGOLIA_INDEX =
  process.env.REACT_APP_ALGOLIA_INDEX || 'asap-hub_dev';
export const SENTRY_DSN = process.env.REACT_APP_SENTRY_DSN;
export const ENVIRONMENT = process.env.REACT_APP_ENVIRONMENT || 'local';
export const RELEASE = process.env.REACT_APP_RELEASE;
export const AUTH0_CLIENT_ID = process.env.REACT_APP_CRN_AUTH0_DOMAIN || '';
export const AUTH0_DOMAIN = process.env.REACT_APP_CRN_AUTH0_CLIENT_ID || '';
