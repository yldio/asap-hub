...
// Base type from the autogenerated query — extended with milestonesCollection
// which is included in the GraphQL query but not yet reflected in codegen output.
// TODO: Remove this extension after running `yarn schema:update:crn` to regenerate types.
type AimsCollectionItemBase = NonNullable<
  NonNullable<ProjectItem['originalGrantAimsCollection']>['items'][number]
>;
type MilestoneCollectionItem = {
  sys: { id: string };
...
const validMilestoneStatuses: MilestoneStatus[] = [
  'Complete',
  'In Progress',
  'Pending',
  'Terminated',
...
/**
 * Extracts a flat, deduplicated list of milestones from aims.
 * Each milestone tracks which aim orders it belongs to (e.g. "1,2,3").
 */
export const parseMilestonesFromAims = (
  items: Array<AimsCollectionItem | null> | undefined,
): Milestone[] | undefined => {
  if (!items) return undefined;
  ...
  const milestoneMap = new Map<
    string,
    { milestone: Milestone; aimOrders: number[] }
  >();
  ...
  items
    ...
    .forEach((aimItem, aimIndex) => {
      const aimOrder = aimIndex + 1;
      const milestoneItems = aimItem.milestonesCollection?.items ?? [];

      milestoneItems
        .filter(
          (ms): ms is MilestoneCollectionItem =>
            ms !== null && !!ms.description?.trim(),
        )
        .forEach((ms) => {
          const { id } = ms.sys;
          const existing = milestoneMap.get(id);

          if (existing) {
            existing.aimOrders.push(aimOrder);
          } else {
            const status = validMilestoneStatuses.includes(
  ...
  if (milestoneMap.size === 0) return undefined;

  return Array.from(milestoneMap.values()).map(({ milestone, aimOrders }) => ({
    ...milestone,
    aims: aimOrders.sort((a, b) => a - b).join(','),
  }));
};
...
export const parseContentfulProjectDetail = (
  ...
  // Aggregate milestones from both original and supplement grant aims
  const allMilestones = [
    ...(parseMilestonesFromAims(item.originalGrantAimsCollection?.items) ?? []),
    ...(parseMilestonesFromAims(item.supplementGrant?.aimsCollection?.items) ??
      []),
  ];
  const milestones = allMilestones.length > 0 ? allMilestones : undefined;

  const originalGrantAims = parseContentfulAims(
    item.originalGrantAimsCollection?.items as
      | Array<AimsCollectionItem | null>
      | undefined,
  );
