import {
  FetchDiscussionTitleAndGranteeParticipantsQuery,
  FetchDiscussionTitleAndGranteeParticipantsQueryVariables,
  FetchManuscriptNotificationDetailsQuery,
  FetchManuscriptNotificationDetailsQueryVariables,
  FETCH_DISCUSSION_TITLE_AND_GRANTEE_PARTICIPANTS,
  ...
  async sendEmailNotification(
    ...
    if (discussionId && action === 'os_member_replied_to_discussion') {
      const { discussions } = await this.contentfulClient.request<
        FetchDiscussionTitleAndGranteeParticipantsQuery,
        FetchDiscussionTitleAndGranteeParticipantsQueryVariables
      >(FETCH_DISCUSSION_TITLE_AND_GRANTEE_PARTICIPANTS, { id: discussionId });
      ...
      if (templateDetails.grantee && allowedRecipients.length >= 1) {
        await this.sendEmail(allowedRecipients, templateDetails.grantee, {
