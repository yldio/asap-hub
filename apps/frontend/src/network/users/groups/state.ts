...
import { groupState } from '@asap-hub/frontend/src/network/groups/state';
...
>({
  ...
  get: (userId) => ({ get }) => {
    ...
    if (index === undefined || index === 'noSuchUser' || index instanceof Error)
      return index;
  ...
  set: (userId) => ({ get, set, reset }, newGroups) => {
