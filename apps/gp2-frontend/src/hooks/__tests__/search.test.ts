...
describe('useSearch', () => {
  describe('property searchQuery', () => {
    ...
    it('resets pagination when changed', async () => {
  ...
  describe('property filters', () => {
    ...
    it('can add a filter', () => {
      ...
      act(() => {
        result.current.toggleFilter('test123', 'filter');
      });
      expect(result.current.filters).toEqual({ filter: ['test123'] });
    });
    ...
    it('can add multiple filters', () => {
      ...
      act(() => {
        result.current.toggleFilter('test456', 'filter');
      });
      expect(result.current.filters).toEqual({
        filter: ['test123', 'test456'],
      });
    ...
    it('resets the pagination when changed', async () => {
      const { result } = renderHook(
        () => ({
          useSearch: useSearch(),
          usePaginationParams: usePaginationParams(),
      ...
      expect(result.current.usePaginationParams.currentPage).toBe(2);
      act(() => {
        result.current.useSearch.toggleFilter('test123', 'filter');
      });
      await waitFor(() => {
        expect(result.current.usePaginationParams.currentPage).toBe(0);
      });
