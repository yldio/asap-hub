...
export class ReminderContentfulDataProvider implements ReminderDataProvider {
  ...
  async fetch(
    ...
    const { outputsCollection, users } = await this.contentfulClient.request<
    ...
    const sortedReminders = publishedOutputReminders.sort(
      (reminderA, reminderB) => {
...
export const getSortDate = (reminder: gp2Model.ReminderDataObject): DateTime =>
  DateTime.fromISO(reminder.data.addedDate);
