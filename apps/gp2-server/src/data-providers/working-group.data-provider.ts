...
import {
  ...
  WorkingGroupContentFragment,
} from '../autogenerated-gql/graphql';
import { FETCH_WORKING_GROUPS } from '../queries/working-groups.queries';
import { createUrl } from '../utils/urls';

type GraphQLWorkingGroupRequiredFlatDataProperties =
  | 'title'
  | 'shortDescription'
  | 'leadingMembers';
type GraphQLWorkingGroupFlatData = WorkingGroupContentFragment['flatData'];
type GraphQLUserRequiredFlatData = Pick<
  GraphQLWorkingGroupFlatData,
  GraphQLWorkingGroupRequiredFlatDataProperties
>;
type GraphQLUserOptionalFlatData = Partial<
  ...
  >
>;
export type GraphQLWorkingGroup = Omit<
  WorkingGroupContentFragment,
  'flatData'
> & {
...
type Member = NonNullable<GraphQLWorkingGroup['flatData']['members']>[number];
...
export function parseWorkingGroupToDataObject(
  ...
  const members =
    workingGroup.flatData.members?.reduce(parseWorkingGroupMembers, []) || [];
