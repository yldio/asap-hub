import { gp2 as gp2Model } from '@asap-hub/model';
import Boom from '@hapi/boom';
import { EventBridgeEvent } from 'aws-lambda';
import { WorkingGroupPayload } from '../../../src/handlers/event-bus';
import { indexWorkingGroupHandler } from '../../../src/handlers/working-group/algolia-index-handler';
import {
  getWorkingGroupEvent,
  getWorkingGroupResponse,
} from '../../fixtures/working-group.fixtures';
import { getAlgoliaSearchClientMock } from '../../mocks/algolia-client.mock';
import { loggerMock } from '../../mocks/logger.mock';
import { workingGroupControllerMock } from '../../mocks/working-group.controller.mock';

const algoliaSearchClientMock = getAlgoliaSearchClientMock();
describe('WorkingGroup index handler', () => {
  const indexHandler = indexWorkingGroupHandler(
    workingGroupControllerMock,
    algoliaSearchClientMock,
    loggerMock,
  );
  beforeEach(jest.resetAllMocks);

  test('Should fetch the workingGroup and create a record in Algolia when workingGroup is published', async () => {
    const workingGroupResponse = getWorkingGroupResponse();
    workingGroupControllerMock.fetchById.mockResolvedValueOnce(
      workingGroupResponse,
    );

    await indexHandler(publishedEvent('42'));

    expect(algoliaSearchClientMock.save).toHaveBeenCalledWith({
      data: expect.objectContaining(workingGroupResponse),
      type: 'working-group',
    });
  });

  test('Should populate the _tags field before saving the workingGroup to Algolia', async () => {
    const workingGroupResponse = getWorkingGroupResponse();
    workingGroupResponse.tags = [{ id: '1', name: 'workingGroup tag' }];
    workingGroupControllerMock.fetchById.mockResolvedValueOnce(
      workingGroupResponse,
    );

    await indexHandler(publishedEvent('42'));
    expect(algoliaSearchClientMock.save).toHaveBeenCalledWith({
      data: {
        ...workingGroupResponse,
        _tags: expect.arrayContaining(['workingGroup tag']),
      },
      type: 'working-group',
    });
  });

  test('Should fetch the workingGroup and remove the record in Algolia when workingGroup is unpublished', async () => {
    const event = unpublishedEvent('42');

    workingGroupControllerMock.fetchById.mockRejectedValue(Boom.notFound());

    await indexHandler(event);

    expect(algoliaSearchClientMock.save).not.toHaveBeenCalled();
    expect(algoliaSearchClientMock.remove).toHaveBeenCalledTimes(1);
    expect(algoliaSearchClientMock.remove).toHaveBeenCalledWith(
      event.detail.resourceId,
    );
  });

  test('Should throw an error and do not trigger algolia when the workingGroup request fails with another error code', async () => {
    workingGroupControllerMock.fetchById.mockRejectedValue(Boom.badData());

    await expect(indexHandler(publishedEvent('42'))).rejects.toThrow(
      Boom.badData(),
    );
    expect(algoliaSearchClientMock.remove).not.toHaveBeenCalled();
  });

  test('Should throw the algolia error when publishing the record fails', async () => {
    const algoliaError = new Error('ERROR');

    workingGroupControllerMock.fetchById.mockResolvedValueOnce(
      getWorkingGroupResponse(),
    );
    algoliaSearchClientMock.save.mockRejectedValueOnce(algoliaError);

    await expect(indexHandler(publishedEvent('42'))).rejects.toThrow(
      algoliaError,
    );
  });

  test('Should throw the algolia error when unpublishing the record fails', async () => {
    const algoliaError = new Error('ERROR');

    workingGroupControllerMock.fetchById.mockRejectedValue(Boom.notFound());

    algoliaSearchClientMock.remove.mockRejectedValueOnce(algoliaError);

    await expect(indexHandler(unpublishedEvent('42'))).rejects.toThrow(
      algoliaError,
    );
  });

  describe('Should process the events, handle race conditions and not rely on the order of the events', () => {
    test('receives the events published and unpublished in correct order', async () => {
      const id = '42';
      const publishedEv = publishedEvent(id);
      const unpublishedEv = unpublishedEvent(id);
      const algoliaError = new Error('ERROR');

      workingGroupControllerMock.fetchById.mockRejectedValue(Boom.notFound());
      algoliaSearchClientMock.remove.mockResolvedValueOnce(undefined);
      algoliaSearchClientMock.remove.mockRejectedValue(algoliaError);

      await indexHandler(publishedEv);
      await expect(indexHandler(unpublishedEv)).rejects.toEqual(algoliaError);

      expect(algoliaSearchClientMock.save).not.toHaveBeenCalled();
      expect(algoliaSearchClientMock.remove).toHaveBeenCalledTimes(2);
      expect(algoliaSearchClientMock.remove).toHaveBeenCalledWith(
        unpublishedEv.detail.resourceId,
      );
    });

    test('receives the events published and unpublished in reverse order', async () => {
      const id = '42';
      const publishedEv = publishedEvent(id);
      const unpublishedEv = unpublishedEvent(id);
      const algoliaError = new Error('ERROR');

      workingGroupControllerMock.fetchById.mockRejectedValue(Boom.notFound());
      algoliaSearchClientMock.remove.mockResolvedValueOnce(undefined);
      algoliaSearchClientMock.remove.mockRejectedValue(algoliaError);

      await indexHandler(unpublishedEv);
      await expect(indexHandler(publishedEv)).rejects.toEqual(algoliaError);

      expect(algoliaSearchClientMock.save).not.toHaveBeenCalled();
      expect(algoliaSearchClientMock.remove).toHaveBeenCalledTimes(2);
      expect(algoliaSearchClientMock.remove).toHaveBeenCalledWith(
        unpublishedEv.detail.resourceId,
      );
    });
  });
});

const unpublishedEvent = (id: string) =>
  getWorkingGroupEvent(id, 'WorkingGroupsUnpublished') as EventBridgeEvent<
    gp2Model.WorkingGroupEvent,
    WorkingGroupPayload
  >;

const publishedEvent = (id: string) =>
  getWorkingGroupEvent(id, 'WorkingGroupsPublished') as EventBridgeEvent<
    gp2Model.WorkingGroupEvent,
    WorkingGroupPayload
  >;
