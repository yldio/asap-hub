...
export SQ_SYNC_SCHEMA_OUTPUT="$(sq sync in packages/squidex/schema -t schemas -t workflows --delete)"
if echo "$SQ_SYNC_SCHEMA_OUTPUT" | grep -q -i -E 'warn|error|fail|exception'; then
    echo "Data import failure - sq sync in packages/squidex/schema -t schemas -t workflows --delete failed:"
...
if [ "$PRINT_SYNC_LOGS_ON_SUCCESS" = true ]; then
    ...
    echo $SQ_SYNC_OUTPUT
    echo "---------------------------------------------"
    echo "sq sync in packages/squidex/schema -t schemas -t workflows --delete successful output:"
    echo $SQ_SYNC_SCHEMA_OUTPUT
fi
