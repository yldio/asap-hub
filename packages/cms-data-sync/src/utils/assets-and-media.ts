...
export const processAndPublishEntry = async (
  ...
  try {
    ...
    if (error instanceof Error) {
      const errorParsed = safeParse(error?.message);
      if (errorParsed?.message === 'File has already been processed.') {
        return;
      }
    }
    logger(
      `Error processing and publishing asset/media: ${entry.sys.id}\n${error}`,
      'ERROR',
    );
  }
};
...
export const checkIfAssetAlreadyExistsInContentful = async (
  ...
  try {
    ...
    logger(
      `Error checking if asset exits in contentful: ${id}\n${error}`,
      'ERROR',
    );

    // assuming the asset does not exist
    // if it is not possible to define it
    return false;
  }
};
...
export const createAndPublishIfNonExistent = async (
  ...
  if (!isAlreadyInContentful) {
    try {
      ...
      logger(
        `Error creating and publishing ${type} : ${id}\n${error}`,
        'ERROR',
      );
    }
  }
