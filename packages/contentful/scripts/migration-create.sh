...
APP_NAME=$APP_NAME CONTENTFUL_SPACE_ID=$CONTENTFUL_SPACE_ID CONTENTFUL_ENV_ID=$CONTENTFUL_ENV_ID CONTENTFUL_ACCESS_TOKEN=$CONTENTFUL_ACCESS_TOKEN CONTENTFUL_MIGRATIONS_DIR=migrations/$APP_NAME yarn workspace @asap-hub/contentful ctf-migrate create $MIGRATION_NAME -c $MIGRATION_TYPE
