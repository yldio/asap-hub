...
export type ResolvedChapter = { startMs: number; title: string };
...
export const resolveChapters = (timeline: Timeline): ResolvedChapter[] => {
  ...
  const fromTitles = placements.flatMap((placement) =>
    placement.clip.kind === 'title' && placement.clip.text.trim()
      ? [{ startMs: placement.startMs, title: placement.clip.text.trim() }]
      : [],
  );

  const fromMarkers = timeline.chapters.flatMap((marker) => {
    const start = startOf(marker.clipId);
    return start === undefined || !marker.title.trim()
      ? []
      : [{ startMs: start + marker.offsetMs, title: marker.title.trim() }];
  });
