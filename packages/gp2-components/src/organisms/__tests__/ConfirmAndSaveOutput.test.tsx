...
import { ConfirmAndSaveOutput, ConfirmAndSaveOutputProps} from '../ConfirmAndSaveOutput';
...
describe('ConfirmAndSaveOutput', () => {
    ...
    const renderElement = (props?: Partial<ConfirmAndSaveOutputProps>) => render(<Form dirty={false}>
      {({ getWrappedOnSave }) => (
        <ConfirmAndSaveOutput
        path={(id: string) => id}
        documentType='Article'
        title='title'
        currentPayload={{...createOutputResponse(),
            tagIds: [],
            contributingCohortIds: [],
            mainEntityId: '', relatedOutputIds: [], relatedEventIds:[],
            authors: []
        }}
        ...
        isEditing={false}
        createVersion={false}
        getWrappedOnSave={getWrappedOnSave as unknown as GetWrappedOnSave<OutputResponse>}
        {...props}
        >
            {({save}) => (
 ...
 it('closes the version modal when user clicks on cancel', async () => {
    ...
    expect(screen.getByText('Publish new version for the whole hub?')).toBeVisible();
