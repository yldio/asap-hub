...
echo "Removing CRN Indexers Stack..."
CRN_INDEXERS_SLS_BUCKET=$(aws-vault exec $AWS_VAULT_PROFILE -- aws cloudformation describe-stack-resource --stack-name=asap-hub-indexers-$PR --logical-resource-id=ServerlessDeploymentBucket | jq .StackResourceDetail.PhysicalResourceId -r)
if [ -z "$CRN_INDEXERS_SLS_BUCKET" ]
then
    echo "No Serverless bucket found"
else
    echo "Removing Serverless bucket '$CRN_INDEXERS_SLS_BUCKET'"
    aws-vault exec $AWS_VAULT_PROFILE -- aws s3 rm s3://$CRN_INDEXERS_SLS_BUCKET --recursive
fi
echo "Deleting stack 'asap-hub-indexers-$PR'"
aws-vault exec $AWS_VAULT_PROFILE -- aws cloudformation delete-stack --stack-name=asap-hub-indexers-$PR --region=us-east-1
echo "Deleted"
