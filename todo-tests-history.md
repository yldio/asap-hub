# React Router v6 Migration - Complete Todo List

> **Status:** In Progress
> **Last Updated:** 2025-11-14
> **Completed:** 1 / 146 files

---

## Table of Contents

1. [React Router v5 to v6 Migration Guide](#migration-guide)
2. [Migration Summary](#migration-summary)
3. [Migration Patterns Reference](#migration-patterns-reference)
4. [Phase 1: Production Code](#phase-1-production-code)
5. [Phase 2: Complex Test Files](#phase-2-complex-test-files)
6. [Phase 3: Medium Complexity Tests](#phase-3-medium-complexity-tests)
7. [Phase 4: Simple Import Updates](#phase-4-simple-import-updates)
8. [Phase 5: Storybook Files](#phase-5-storybook-files)
...
<details>
<summary><strong>Click to expand: React Router v5 to v6 Official Migration Guide</strong></summary>
...
3. Remove `<Redirect>`s inside `<Switch>`
4. Refactor custom `<Route>`s
5. Upgrade to React Router v6
...
### Overview
- **Total Files Requiring Changes:** 146
- **Production Code Files:** 3 (CRITICAL - affects all apps)
- **Complex Test Files:** 11 (require full migration pattern)
- **Medium Complexity Tests:** 30+ (Router + createMemoryHistory replacement)
- **Simple Import Updates:** 70+ (StaticRouter import path only)
- **Storybook Files:** 31 (lowest priority)

### Completed
- ✅ `apps/gp2-frontend/src/auth/__tests__/Signin.test.tsx` (Reference Implementation)

### Effort Estimate
- **Phase 1 (Production):** 1-2 hours
- **Phase 2 (Complex Tests):** 4-6 hours
- **Phase 3 (Medium Tests):** 3-4 hours
- **Phase 4 (Simple Imports):** 1-2 hours (can be automated)
- **Phase 5 (Storybook):** 30 minutes
...
### Pattern 1: Remove History Package Imports
...
**After (v6):**
```typescript
// Remove entirely - use React Router v6 hooks instead
import { useNavigate, useLocation, useParams } from 'react-router-dom';
```

---

### Pattern 2: Update StaticRouter Import Path
...
### Pattern 3: Replace Router + createMemoryHistory with MemoryRouter

**Before (v5):**
```typescript
const history = createMemoryHistory({
  initialEntries: ['/some-path?query=value'],
});
...
```typescript
render(
  <MemoryRouter initialEntries={['/some-path?query=value']}>
    <Component />
  </MemoryRouter>
);

// Access location via hook inside component
function Component() {
  const location = useLocation();
  const navigate = useNavigate();
...
### Pattern 4: LocationCapture Component Pattern

When you need to access location in tests (replacing `history.location`):
...
render(
  <MemoryRouter initialEntries={['/path?search=value']}>
    <LocationCapture />
    <YourComponent />
  </MemoryRouter>
);

// In tests, access locationRef.current instead of history.location
```

---

### Pattern 5: React Router v6→v7 Deprecation Warning Suppression

Add to test files that use MemoryRouter:

```typescript
// At the top level or in describe block
beforeEach(() => {
  // Explicitly mock console.warn to suppress React Router v6->v7 deprecation warnings
...
### Pattern 6: Async UserEvent Patterns
...
And use `waitFor` for assertions:

```typescript
await waitFor(() => {
  expect(container).not.toHaveTextContent(/problem/i);
});
```
...
**Priority: CRITICAL** | **Status: Not Started** | **Estimated Time: 1-2 hours**

> ⚠️ **WARNING:** These files affect production code. Test thoroughly after changes!

### 1.1 Layout Components (Type Imports Only)

#### File: `packages/react-components/src/templates/Layout.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { Location } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:**
  - Remove `Location` type import from 'history'
  - Use React Router's `Location` type from `useLocation()` return type
  - Update all `Location` type references
- **Complexity:** Simple
- **Notes:** Component already uses `useLocation()` hook, only type imports need updating
...
#### File: `packages/gp2-components/src/templates/Layout.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { Location } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:**
  - Same as react-components Layout.tsx above
- **Complexity:** Simple
- **Notes:** Identical pattern to react-components version

---

#### File: `packages/gp2-components/src/molecules/UserMenu.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { Location } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:**
  - Remove `Location` type import from 'history'
  - Replace with React Router's location type
  - Component uses `useLocation()` for navigation tracking
- **Complexity:** Simple
- **Notes:** Only type imports, no runtime logic changes

---

## Phase 2: Complex Test Files
**Priority: HIGH** | **Status: 1/11 Completed** | **Estimated Time: 4-6 hours**

> 🔧 These files require the full migration pattern including LocationCapture, warning suppression, and async updates.

### 2.1 Authentication Tests

#### File: `apps/crn-frontend/src/auth/__tests__/Signin.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory, History } from 'history';`
- **StaticRouter import:** `import { Router, StaticRouter } from 'react-router-dom';`
- **Changes needed:**
  - Remove history package imports
  - Update StaticRouter import to `'react-router-dom/server'`
  - Replace `Router + createMemoryHistory` with `MemoryRouter`
  - Create `LocationCapture` component
  - Add deprecation warning suppression
  - Make `userEvent.click()` async
  - Add `waitFor` for async assertions
- **Complexity:** Complex
- **Reference:** Same as completed gp2-frontend/Signin.test.tsx

---

#### File: `apps/gp2-frontend/src/auth/__tests__/Signin.test.tsx`
- **Status:** ✅ Completed (Reference Implementation)
- **Notes:** Use this as the pattern for all other complex migrations
- **Key Patterns:**
  - LocationCapture component with `useLocation()` hook
  - `beforeEach` mock for React Router v6→v7 warnings
  - Async userEvent calls
  - `waitFor` in assertions

---

#### File: `apps/crn-frontend/src/auth/__tests__/CheckOnboarded.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createBrowserHistory, History } from 'history';`
- **StaticRouter import:** N/A (uses `Router` with history)
- **Changes needed:**
  - Remove history package
  - Replace `Router + createBrowserHistory` with `MemoryRouter`
  - Tests use `history.push()` for navigation - refactor to use navigate from hook
  - Multiple test cases rely on `history.push()`
- **Complexity:** Medium-Complex
- **Notes:** Uses createBrowserHistory instead of createMemoryHistory, but same approach applies

---

### 2.2 Routing Utility Tests

#### File: `packages/frontend-utils/src/hooks/__tests__/routing.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createBrowserHistory } from 'history';`
- **StaticRouter import:** `import { Router, StaticRouter } from 'react-router-dom';`
- **Changes needed:**
  - Remove history package
  - Update StaticRouter import path
  - Replace `Router + createBrowserHistory/createMemoryHistory` with `MemoryRouter`
  - Tests custom routing hooks - may need significant refactoring
- **Complexity:** Medium-Complex
- **Notes:** Tests fundamental routing hooks, needs careful attention

---

#### File: `packages/frontend-utils/src/structure/__tests__/ErrorBoundary.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory } from 'history';`
- **StaticRouter import:** `import { Route, Router } from 'react-router-dom';`
- **Changes needed:**
  - Remove history package
  - Replace `Router + createMemoryHistory` with `MemoryRouter`
  - Uses `history.push()` to test error boundary reset on navigation
  - Replace Routes syntax with v6 (element prop)
- **Complexity:** Simple-Medium
- **Notes:** Single navigation test case, straightforward migration

---

#### File: `packages/react-components/src/__tests__/routing.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory } from 'history';`
- **StaticRouter import:** `import { Router, StaticRouter } from 'react-router-dom';`
- **Changes needed:**
  ...
  - Update StaticRouter import path
  - Replace `Router + createMemoryHistory` with `MemoryRouter`
  - Uses `history.push()` and `history.location` for assertions
- **Complexity:** Medium
- **Notes:** Tests routing utility functions

---

### 2.3 Component Navigation Tests

#### File: `packages/react-components/src/atoms/__tests__/NavigationLink.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory } from 'history';`
- **StaticRouter import:** `import { Router, StaticRouter } from 'react-router-dom';`
- **Changes needed:**
  ...
  - Update StaticRouter import path
  - Replace `Router + createMemoryHistory` with `MemoryRouter`
  - Uses `history.location.pathname` for assertions - replace with LocationCapture
- **Complexity:** Medium
- **Notes:** Tests navigation component behavior

---

### 2.4 Form Submission Tests

#### File: `packages/react-components/src/templates/__tests__/TeamProfileWorkspace.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:**
  - Remove history package
  - Replace `Router + createMemoryHistory` with `MemoryRouter`
  - Tests redirect behavior after form submission
  - Uses `history.location.pathname` for assertions
- **Complexity:** Medium
- **Notes:** Tests navigation after user actions

---

#### File: `packages/react-components/src/templates/__tests__/ManuscriptForm.basic.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory, History } from 'history';`
- **StaticRouter import:** `import { MemoryRouter, Route, Router, StaticRouter } from 'react-router-dom';`
- **Changes needed:**
  - Remove `History` type import
  - Update StaticRouter import path
  - Replace `Router + createMemoryHistory` with `MemoryRouter` where applicable
  - File already uses StaticRouter for some tests
- **Complexity:** Medium
- **Notes:** Mix of StaticRouter and Router+history usage

---

### 2.5 Research Output Form Tests (10 files)

All following files have the same pattern:

#### Files:
- `packages/react-components/src/templates/__tests__/ResearchOutputForm/ResearchOutputForm.1.test.tsx`
- `packages/react-components/src/templates/__tests__/ResearchOutputForm/ResearchOutputForm.2.test.tsx`
- `packages/react-components/src/templates/__tests__/ResearchOutputForm/ResearchOutputForm.3.test.tsx`
- `packages/react-components/src/templates/__tests__/ResearchOutputForm/ResearchOutputForm.4.test.tsx`
- `packages/react-components/src/templates/__tests__/ResearchOutputForm/ResearchOutputForm.5.test.tsx`
- `packages/react-components/src/templates/__tests__/ResearchOutputForm/ResearchOutputForm.6.test.tsx`
- `packages/react-components/src/templates/__tests__/ResearchOutputForm/ResearchOutputForm.7.test.tsx`
- `packages/react-components/src/templates/__tests__/ResearchOutputForm/ResearchOutputForm.8.test.tsx`
- `packages/react-components/src/templates/__tests__/ResearchOutputForm/ResearchOutputForm.9.test.tsx`
- `packages/react-components/src/templates/__tests__/ResearchOutputForm/ResearchOutputForm.10.test.tsx`
...
- **Changes needed:**
  ...
  - Replace `Router + createMemoryHistory` with `MemoryRouter`
  - Update test setup
- **Complexity:** Simple (same pattern across all files)
- **Notes:** Can potentially be migrated in batch since they follow identical pattern

---

## Phase 3: Medium Complexity Tests
**Priority: MEDIUM** | **Status: Not Started** | **Estimated Time: 3-4 hours**

> 📝 These files use Router + createMemoryHistory but with simpler test patterns.

### 3.1 CRN Frontend Team Tests

#### File: `apps/crn-frontend/src/network/teams/__tests__/TeamManuscript.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory, MemoryHistory } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:**
  - Remove history package
  - Replace `Router + createMemoryHistory` with `MemoryRouter`
  - Uses `history.push()` for navigation in tests
- **Complexity:** Medium

---

#### File: `apps/crn-frontend/src/network/teams/__tests__/TeamManuscript2.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory, MemoryHistory } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:** Same as TeamManuscript.test.tsx
- **Complexity:** Medium

---

#### File: `apps/crn-frontend/src/network/teams/__tests__/TeamManuscript3.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory, MemoryHistory } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:** Same as TeamManuscript.test.tsx
...
#### File: `apps/crn-frontend/src/network/teams/__tests__/TeamManuscript4.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory, MemoryHistory } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:** Same as TeamManuscript.test.tsx
...
#### File: `apps/crn-frontend/src/network/teams/__tests__/TeamManuscript5.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory, MemoryHistory } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:** Same as TeamManuscript.test.tsx
...
#### File: `apps/crn-frontend/src/network/teams/__tests__/TeamComplianceReport.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:** Same as TeamManuscript.test.tsx
...
#### File: `apps/crn-frontend/src/network/teams/__tests__/TeamOutput.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory, History } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:** Same as TeamManuscript.test.tsx
...
#### File: `apps/crn-frontend/src/network/teams/__tests__/TeamProfile.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:** Same as TeamManuscript.test.tsx
...
### 3.2 Working Group Tests

#### File: `apps/crn-frontend/src/network/working-groups/__tests__/WorkingGroupOutput.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory, History } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:**
...
#### File: `apps/crn-frontend/src/network/working-groups/__tests__/WorkingGroupProfile.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:** Same as WorkingGroupOutput.test.tsx
- **Complexity:** Medium
...
#### File: `apps/crn-frontend/src/analytics/open-science/__tests__/OpenScience.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:**
...
### 3.4 Component Tests with History

#### File: `packages/react-components/src/organisms/__tests__/ManuscriptCard.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:**
...
#### File: `packages/react-components/src/organisms/__tests__/Form.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory, History } from 'history';`
- **StaticRouter import:** `import { Router, StaticRouter } from 'react-router-dom';`
- **Changes needed:**
...
#### File: `packages/react-components/src/organisms/__tests__/ManuscriptVersionCard.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:**
...
#### File: `packages/gp2-components/src/organisms/__tests__/ResourceModal.test.tsx`
- **Status:** ⬜ Not Started
- **Current imports from 'history':** `import { createMemoryHistory } from 'history';`
- **StaticRouter import:** N/A
- **Changes needed:**
...
## Phase 4: Simple Import Updates
**Priority: MEDIUM-LOW** | **Status: Not Started** | **Estimated Time: 1-2 hours**

> ✅ These files only need StaticRouter import path update. Can be done with find/replace.

**Pattern:** Change all occurrences of:
```typescript
import { StaticRouter } from 'react-router-dom';
```
to:
...
### 4.1 CRN Frontend Tests

- ⬜ `apps/crn-frontend/src/__tests__/AuthenticatedApp.test.tsx`
- ⬜ `apps/crn-frontend/src/events/__tests__/Event.test.tsx`
- ⬜ `apps/crn-frontend/src/groups/__tests__/Group.test.tsx`
- ⬜ `apps/crn-frontend/src/groups/__tests__/GroupProfileAbout.test.tsx`
- ⬜ `apps/crn-frontend/src/groups/__tests__/GroupProfileCalendar.test.tsx`
- ⬜ `apps/crn-frontend/src/groups/__tests__/GroupProfileOutputs.test.tsx`
- ⬜ `apps/crn-frontend/src/groups/__tests__/GroupProfileWorkspace.test.tsx`

---

### 4.2 GP2 Frontend Tests

- ⬜ `apps/gp2-frontend/src/__tests__/AuthenticatedApp.test.tsx`
- ⬜ `apps/gp2-frontend/src/__tests__/OnboardedApp.test.tsx`
- ⬜ `apps/gp2-frontend/src/outputs/__tests__/OutputDirectory.test.tsx`
- ⬜ `apps/gp2-frontend/src/projects/__tests__/Project.test.tsx`
- ⬜ `apps/gp2-frontend/src/users/__tests__/UserProfile.test.tsx`
- ⬜ `apps/gp2-frontend/src/working-groups/__tests__/WorkingGroup.test.tsx`

---

### 4.3 React Components Tests

- ⬜ `packages/react-components/src/atoms/__tests__/Anchor.test.tsx`
- ⬜ `packages/react-components/src/atoms/__tests__/ScrollToTop.test.tsx`
- ⬜ `packages/react-components/src/atoms/__tests__/TabLink.test.tsx`
- ⬜ `packages/react-components/src/molecules/__tests__/ModalEditHeaderDecorator.test.tsx`
- ⬜ `packages/react-components/src/organisms/__tests__/ConfirmModal.test.tsx`
- ⬜ `packages/react-components/src/organisms/__tests__/MainNavigation.test.tsx`
- ⬜ `packages/react-components/src/organisms/__tests__/ResearchOutputContributorsCard.test.tsx`
- ⬜ `packages/react-components/src/organisms/__tests__/ResearchOutputRelatedEventsCard.test.tsx`
- ⬜ `packages/react-components/src/organisms/__tests__/ResearchOutputRelatedResearchCard.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/BiographyModal.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/ComplianceReportForm.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/ContactInfoModal.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/EventsPageHeader.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/ExpertiseAndResourcesModal.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/InterestGroupProfileHeader.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/ManuscriptForm.errors.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/ManuscriptForm.lifecycle.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/ManuscriptForm.other.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/ManuscriptForm.quickChecks.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/ManuscriptForm.validation.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/NetworkPageHeader.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/OpenQuestionsModal.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/PersonalInfoModal.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/ProjectsPageHeader.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/ResearchOutputForm/ResearchOutputForm.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/RoleModal.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/SharedResearchList.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/ToolModal.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/UserProfileHeader.test.tsx`

---

### 4.4 GP2 Components Tests

- ⬜ `packages/gp2-components/src/molecules/__tests__/OutputSharingCards.test.tsx`
- ⬜ `packages/gp2-components/src/organisms/__tests__/DashboardPageBody.test.tsx`
- ⬜ `packages/gp2-components/src/organisms/__tests__/OutputForm.test.tsx`
- ⬜ `packages/gp2-components/src/organisms/__tests__/UserMenu.test.tsx`
- ⬜ `packages/gp2-components/src/templates/__tests__/BiographyModal.test.tsx`
- ⬜ `packages/gp2-components/src/templates/__tests__/ContactInformationModal.test.tsx`
- ⬜ `packages/gp2-components/src/templates/__tests__/KeyInformationModal.test.tsx`
- ⬜ `packages/gp2-components/src/templates/__tests__/Layout.test.tsx`
- ⬜ `packages/gp2-components/src/templates/__tests__/OutputDirectory.test.tsx`
- ⬜ `packages/gp2-components/src/templates/__tests__/ProjectDetailHeader.test.tsx`
- ⬜ `packages/gp2-components/src/templates/__tests__/ProjectsPageBody.test.tsx`
- ⬜ `packages/gp2-components/src/templates/__tests__/TagsModal.test.tsx`
- ⬜ `packages/gp2-components/src/templates/__tests__/UserDetailHeaderCard.test.tsx`
- ⬜ `packages/gp2-components/src/templates/__tests__/WorkingGroupAbout.test.tsx`
- ⬜ `packages/gp2-components/src/templates/__tests__/WorkingGroupDetailHeader.test.tsx`

---

### 4.5 Additional Test Files

- ⬜ `apps/crn-frontend/src/network/__tests__/ProfileInterestGroups.test.tsx`
- ⬜ `apps/crn-frontend/src/network/__tests__/UsersList.test.tsx`
- ⬜ `apps/crn-frontend/src/network/users/__tests__/About.test.tsx`
- ⬜ `apps/crn-frontend/src/network/users/__tests__/UserProfile.test.tsx`
- ⬜ `apps/gp2-frontend/src/network/__tests__/UsersList.test.tsx`
- ⬜ `apps/gp2-frontend/src/network/users/__tests__/About.test.tsx`
- ⬜ `apps/gp2-frontend/src/network/users/__tests__/UserProfile.test.tsx`
- ⬜ `apps/gp2-frontend/src/shared-research/__tests__/OutputDirectory.test.tsx`
- ⬜ `packages/gp2-components/src/molecules/__tests__/ProjectContributorsCard.test.tsx`
- ⬜ `packages/react-components/src/organisms/__tests__/ActivityCard.test.tsx`
- ⬜ `packages/react-components/src/organisms/__tests__/InterestGroupCard.test.tsx`
- ⬜ `packages/react-components/src/organisms/__tests__/TeamCard.test.tsx`
- ⬜ `packages/react-components/src/organisms/__tests__/UserProfilePersonalText.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/EventPage.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/InterestGroupAbout.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/TeamAbout.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/TeamWorkspace.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/WorkingGroupAbout.test.tsx`
- ⬜ `packages/react-components/src/templates/__tests__/WorkingGroupWorkspace.test.tsx`

---

## Phase 5: Storybook Files
**Priority: LOW** | **Status: Not Started** | **Estimated Time: 30 minutes**

> 📚 Storybook files - same simple import update as Phase 4.

**Pattern:** Change StaticRouter import path only.

### 5.1 Storybook Stories

- ⬜ `apps/storybook/src/NetworkPage.stories.tsx`
- ⬜ `apps/storybook/src/BiographyModal.stories.tsx`
- ⬜ `apps/storybook/src/ConfirmModal.stories.tsx`
- ⬜ `apps/storybook/src/ContactInfoModal.stories.tsx`
- ⬜ `apps/storybook/src/EventsPage.stories.tsx`
- ⬜ `apps/storybook/src/InterestGroupProfilePage.stories.tsx`
- ⬜ `apps/storybook/src/MainNavigation.stories.tsx`
- ⬜ `apps/storybook/src/OpenQuestionsModal.stories.tsx`
- ⬜ `apps/storybook/src/PersonalInfoModal.stories.tsx`
- ⬜ `apps/storybook/src/ResearchOutputForm.stories.tsx`
- ⬜ `apps/storybook/src/RoleModal.stories.tsx`
- ⬜ `apps/storybook/src/SharedResearchList.stories.tsx`
- ⬜ `apps/storybook/src/TabLink.stories.tsx`
- ⬜ `apps/storybook/src/TabNav.stories.tsx`
- ⬜ `apps/storybook/src/TeamProfilePage.stories.tsx`
- ⬜ `apps/storybook/src/ToolModal.stories.tsx`
- ⬜ `apps/storybook/src/UserNavigation.stories.tsx`
- ⬜ `apps/storybook/src/UserProfileExpertiseAndResourcesModal.stories.tsx`
- ⬜ `apps/storybook/src/UserProfilePage.stories.tsx`
- ⬜ `apps/storybook/src/WorkingGroupPage.stories.tsx`
- ⬜ `apps/storybook/src/gp2-components/BiographyModal.stories.tsx`
- ⬜ `apps/storybook/src/gp2-components/ContactInformationModal.stories.tsx`
- ⬜ `apps/storybook/src/gp2-components/KeyInformationModal.stories.tsx`
- ⬜ `apps/storybook/src/gp2-components/OutputDirectory.stories.tsx`
- ⬜ `apps/storybook/src/gp2-components/OutputPage.stories.tsx`
- ⬜ `apps/storybook/src/gp2-components/ProjectDetailPage.stories.tsx`
- ⬜ `apps/storybook/src/gp2-components/ProjectsPage.stories.tsx`
- ⬜ `apps/storybook/src/gp2-components/TagsModal.stories.tsx`
- ⬜ `apps/storybook/src/gp2-components/UserProfileAbout.stories.tsx`
- ⬜ `apps/storybook/src/gp2-components/WorkingGroupPage.stories.tsx`
- ⬜ `apps/storybook/src/gp2-components/WorkingGroupsPage.stories.tsx`
...
2. **Check for Warnings:**
   - Look for React Router v6→v7 deprecation warnings
   - Verify warning suppression is working

3. **Manual Testing:**
   - For production code (Phase 1): Test all affected pages manually
   - For Storybook (Phase 5): Verify all stories still render
...
### Phase 4 & 5 (Simple Imports)
Can be automated with a bash script:

```bash
#!/bin/bash
# Update StaticRouter imports

find apps packages -name "*.tsx" -o -name "*.ts" | xargs sed -i '' \
  's/import { \(.*\)StaticRouter\(.*\) } from '\''react-router-dom'\'';/import { \1StaticRouter\2 } from '\''react-router-dom\/server'\'';/g'
```

Or use a Node script with `jscodeshift` for safer refactoring.

---

## Notes & Gotchas

1. **History Package Removal:**
   - The `history` package is a dependency of React Router v6, but it's not exposed
   - Don't import directly from `history` - use React Router hooks instead

2. **Location Type:**
   - Don't import `Location` type from 'history'
   - Use `ReturnType<typeof useLocation>` or define your own type
...
4. **Route Syntax:**
   - `<Route path="/users/:id">` → `<Route path="/users/:id" element={<Component />} />`
   - No more `exact` prop - routes are exact by default
   - Use trailing `*` for nested routes

5. **Testing:**
   - Always test navigation flows after migration
   - Watch for console warnings about future flags
   - Verify all userEvent calls are awaited
...
- [React Router v6 API Reference](https://reactrouter.com/en/main)
- [Completed Example: apps/gp2-frontend/src/auth/__tests__/Signin.test.tsx](apps/gp2-frontend/src/auth/__tests__/Signin.test.tsx)
